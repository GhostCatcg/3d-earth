...
function createFlyLine(r, startAngle, endAngle) {
  ...
  //  ArcCurve创建圆弧曲线
  const arc = new ArcCurve(0, 0, r, startAngle, endAngle, false);
  ...
  for (const i = 0; i < pointsArr.length; i++) {
  ...
  for (let i = 0; i < pointsArr.length; i++) {
    const color1 = new Color(basis.flyLine.colorStart); //轨迹线颜色 青色
    const color2 = new Color(basis.flyLine.colorEnd); //黄色
  ...
  // const material = new  LineBasicMaterial({color: 0xffff00,});//线条材质
  // const line = new  Line(geometry, material); //线条模型对象
  // let point = arc.getPoint(0.1)
  ...
  // mesh.rotateX(360)
  // mesh.position.copy(FlyLine.position)
  // FlyLine.add(mesh)
  // const sphereGeo = new  SphereGeometry(0.01, 30, 30);
  // const materialGeo = new  MeshBasicMaterial({
  //   color: 0xffff00
  // });
  // sphereGeo.setFromPoints(arc)
  // const sphere = new  Mesh(sphereGeo, materialGeo);
 ...
 */
function flyArc(radius, lon1, lat1, lon2, lat2) {
...
function arcXOY(startPoint, endPoint) {
  ...
  const arcTopCoord = dir.multiplyScalar(R + earthRadianAngle * R * 0.2) // 黄色飞行线的高度
...
function radianAOB(A, B, O) {
  ...
  // console.log('夹角度数', Math.radToDeg(radianAngle));
  return radianAngle
}
/*绘制一条圆弧曲线模型Line
5个参数含义：(圆心横坐标, 圆心纵坐标, 飞线圆弧轨迹半径, 开始角度, 结束角度)*/
function circleLine(x, y, r, startAngle, endAngle) {
  const geometry = new Geometry(); //声明一个几何体对象Geometry
  //  ArcCurve创建圆弧曲线
